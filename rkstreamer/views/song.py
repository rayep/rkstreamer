...
class JioSaavnSongView(ISongView):
    ...
    def display_rsong_queue(self, queue: list[SongType]) -> None:
    ...
    def play_media(self, song: SongType) -> None:
        ...
        return self.media_player.play_media(song.stream_url)

    def remove_media(self, song: SongType) -> None:
        """Removes media from media list"""
        return self.media_player.remove_media(song.stream_url)

    def add_media(self, song: SongType) -> None:
        """Adding media to media list"""
        return self.media_player.add_media(song.stream_url)

    def player_input(self, user_input: str) -> None:
        """Player input"""
        self.player.player_controls(user_input)

    def set_monitor_callback(self, callback_fn: Callable) -> None:
